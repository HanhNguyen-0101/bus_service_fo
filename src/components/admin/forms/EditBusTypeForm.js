...
import { editBusType } from "../../../redux/actions/GlobalAction";

export default function EditBusTypeForm() {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const { busTypeSelected } = useSelector((state) => state.GlobalReducer);
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      id: busTypeSelected?.id,
      name: busTypeSelected?.name,
    },
