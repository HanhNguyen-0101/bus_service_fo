...
import { editOrderStatus } from "../../../redux/actions/GlobalAction";

export default function EditOrderStatusForm() {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const { orderStatusSelected } = useSelector((state) => state.GlobalReducer);
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      id: orderStatusSelected?.id,
      name: orderStatusSelected?.name,
    },
    validationSchema: Yup.object({
      name: Yup.string()
        .max(30, t("maxCharError", {max: 30}))
        .required(t("pleaseInput", {title: t("orderStatus")})),
    }),
    onSubmit: (values) => {
      dispatch(editOrderStatus(values));
    },
  });
