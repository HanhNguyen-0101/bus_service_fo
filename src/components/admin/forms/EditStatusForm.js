...
import { editStatus } from "../../../redux/actions/GlobalAction";

export default function EditStatusForm() {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const { statusSelected } = useSelector((state) => state.GlobalReducer);
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string()
        ...
        .required(t("pleaseInputName", {title: t("status")})),
    }),
    onSubmit: (values) => {
      ...
      <Form.Item
        ...
        label={
          <span className="font-bold capitalize">
            {t("name", {title: t("status")})}<span className="text-red-500">*</span>
          </span>
        }
      >
        <Input
