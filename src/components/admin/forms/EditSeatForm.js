...
import { editSeat, getAllSeat } from "../../../redux/actions/SeatAction";
...
export default function EditSeatForm() {
  ...
  const { seatList, seatSelected } = useSelector((state) => state.SeatReducer);
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      vehicledId: seatSelected?.vehicledId,
      seatStatusId: seatSelected?.seatStatusId,
    },
  ...
  return (
    ...
    >
      ...
      {seatByVehicle && (
        <>
          <div className="font-bold mb-1 capitalize">{t("busList")}</div>
          <div>
